	...
	*abortflag = 0;
	...
	while(1)
	{
		...
		for(c = 0; c < trainlength; c++)
		{
			...
			ret = sendtowrapper(udpsock, buf, 1400, 0, 
					(struct sockaddr *)from, fromlen, 
					capacity, sendts, sleepRes);
			if(ret == -1)
			{
				perror("cannot send\n");
				close(udpsock);
		...
		double *tbrate, unsigned int *abortflag, FILE *fp)
{
	...
	fprintf(fp, "probe trainlength: %d\n", trainlength);
printf("probe trainlength: %d\n", trainlength);
		...
		unsigned int *abortflag, FILE *fp)
{
	...
	*abortflag = endpkt.abortflag = endpkt.result = 0;
	...
	/*for(ret = 0; ret < index; ret++)
	{
		printf("rate %f rank: %d loss rate %f\n", 
			rate[ret], rank[ret], lossrate[ret]);
	}*/
