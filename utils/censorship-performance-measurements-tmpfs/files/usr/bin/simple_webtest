...
url_dload_address=ben.noise.gatech.edu/censorship-performance/simple-http
...
iteration_dload_cap=`expr 10 \* 1024` #15k
...
pick_random_url()
{
    . $index_file #this is necessary to keep consistent values of index. I think it might be a variable scope issue, but this works
    cur_loc=0
    ...
    #export the index variable to disk and the name of the input file
    index=`expr $index + 1`
...
run_measurements()
{
    ...
    data_usage=0
    while [ $data_usage -lt $iteration_dload_cap ]; do
	url=`pick_random_url`
	echo $url
	measure_site $url
	compress_data $url
